...
bool isFileExist(string filename, struct filesystem fileSystem) {
    ...
    string fileSystemName = fileSystem.fileSystemName;
...
void removeFunc(string filename, struct filesystem fileSystem) {
    ...
    if (findDesc.firstBlockIndex == EMPTY_FILE) {
        ...
        write(fd, &findBlock, sizeof(findBlock));
        close(fd);
        clearBlockFunc(firstBlockIndex, fileSystem);
        return;
    }
}
...
void writeFunc(string filename, string message, struct filesystem fileSystem) {
    ...
    if (firstBlockIndex == EMPTY_FILE) {
        int offset = findFreeBlock(fileSystem);
        if (offset == -1) {
    ...
    write(message, firstBlockIndex, fileSystem);
}
...
string readFunc(string filename, struct filesystem fileSystem) {
    ...
    return read(firstBlockIndex, fileSystem);
}
