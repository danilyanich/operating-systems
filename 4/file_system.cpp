...
Folder *file_system::find_folder(const string &name) {
    vector<string> find_folder_path;//абсолютный путь к искомой папке format: /root/path/to/dir
    string find_folder_name = name.substr(name.find_last_of('/') + 1);//name of dir for finding
    if (this->root->name == find_folder_name)// if name == /root
        return this->root;
    find_folder_path.emplace(find_folder_path.begin(), find_folder_name);//add dir to path
    ...
    }//fill find_folder_path like: if name = /root/dir1/dir2, then find_folder_path = {dir1,dir2}
...
int file_system::move_folder(User *user, Folder *folder_to_move, Folder *destination_folder) {
    ...
    //then move to destination_folder
...
int file_system::delete_file(User *user, File *file_to_delete) {
    ...
    //clear blocks(delete from memory)
...
int file_system::find_block(int start_from, int size_of_memory) {
    ...
    //if not enough memory for file then allocate more memory
...
int file_system::copy_file(User *user, File *file_to_copy, Folder *destination_folder) {
    ...
    //copy data from file_to_copy to new_file
...
void file_system::print_file_system() {
    ...
    //auto currFol_name = curr_folder_name;
    curr_folder_name.insert(curr_folder_name.begin(), this->currFolder->name);
    int level = 0;
    vector<int> places;
