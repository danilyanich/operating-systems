...
m_id m_malloc(int size_of_segment, m_err_code* error) {
  ...
  if (_g_allocator_memory->next == NULL) {                       // добавляем сегмент в начало, если он единственный
...
void m_init(int size_of_memory, m_err_code* error) {
  ...
  _g_allocator_memory = calloc(1, sizeof(memory));  // инициируем список сегментов
  _g_allocator_memory->size = size_of_memory;
  _g_allocator_memory->memory = calloc(1, size_of_memory);  // инициируем память
