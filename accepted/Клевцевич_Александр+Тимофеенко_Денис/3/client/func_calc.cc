...
class FuncCalcClient {
   public:
    ...
    double CalcFactorialFunc(double x) {
        ...
        Status status = stub->CalculateFactorialFunc(&context, request, &reply);
...
int main(int argc, char **argv) {
    ...
    std::cin >> x;
    try {
        res = calculator.CalcFactorialFunc(x);
    } catch (const std::exception &e) {
        std::cerr << e.what() << '\n';
        return 1;
    }

    std::cout << "Result of factorial function with x=" << x << " result is "
              << res << std::endl;
}
