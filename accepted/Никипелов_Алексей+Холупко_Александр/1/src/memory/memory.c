...
char* g_allocator_memory = NULL;
int g_allocator_memory_size = 0;
int g_bytes_allocated = 0;
...
m_id m_malloc(int size_of_chunk, m_err_code* error) {
    if (g_bytes_allocated + size_of_chunk > g_allocator_memory_size) {
        *error = M_ERR_ALLOCATION_OUT_OF_MEMORY;
        return NULL;
    }

    g_bytes_allocated += size_of_chunk;
...
void m_init(int number_of_pages, int size_of_page) {
  if (g_allocator_memory != NULL) free(g_allocator_memory);

  g_allocator_memory_size = number_of_pages * size_of_page;
  g_allocator_memory = malloc(g_allocator_memory_size);
  g_bytes_allocated = 0;
